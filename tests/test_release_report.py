...
class ReleaseReportTests(unittest.TestCase):
    def test_release_evidence_is_synchronized(self):
        ...
        report = json.loads((ROOT / "analysis" / "ao-release-header-report.json").read_text(encoding="utf-8"))
        with (ROOT / "research" / "releases.csv").open(newline="", encoding="utf-8") as stream:
        ...
        self.assertEqual(len(project_by_id), 6)
