#!/usr/bin/env python3
"""Extract BLUE player fishing sprite batch 002 from the six source ROMs.

Outputs one deduplicated shared copy of each native 2bpp asset and a human-viewable
PNG. Every source ROM location is verified against the same bytes before the shared
asset is written. ROM binaries are inputs only and are never copied to the output.
"""
...
ASSETS = {
    "red_fish_front": {
        "width": 16, "height": 8, "length": 32,
        "offsets": {"jp": 0x7A017, "en": 0x79FED, "de": 0x79FED, "fr": 0x79FED, "it": 0x79FED, "es": 0x79FED},
        "bpp_sha1": "32f54d205f7a65a99cda69e4326546969df261b4",
    },
    "red_fish_back": {
        "width": 16, "height": 8, "length": 32,
        "offsets": {"jp": 0x7A037, "en": 0x7A00D, "de": 0x7A00D, "fr": 0x7A00D, "it": 0x7A00D, "es": 0x7A00D},
        "bpp_sha1": "5ee917dcefa179fff30f23e9b83fd8e360b4a55d",
    },
    "red_fish_side": {
        "width": 16, "height": 8, "length": 32,
        "offsets": {"jp": 0x7A057, "en": 0x7A02D, "de": 0x7A02D, "fr": 0x7A02D, "it": 0x7A02D, "es": 0x7A02D},
        "bpp_sha1": "d220164f22280516334e1d353896603577b2c5f1",
    },
    "fishing_rod": {
        "width": 8, "height": 24, "length": 48,
        "offsets": {"jp": 0x7A077, "en": 0x7A04D, "de": 0x7A04D, "fr": 0x7A04D, "it": 0x7A04D, "es": 0x7A04D},
        "bpp_sha1": "f6cfd253704dfba1f6e747ca4378211505110ebf",
    },
}
...
def bpp_to_pixels(data: bytes, width: int, height: int) -> list[int]:
    ...
    if len(data) != width_tiles * height_tiles * 16:
    ...
    for ty in range(height_tiles):
        for tx in range(width_tiles):
            ...
            for y in range(8):
                ...
                for x in range(8):
                    bit = 7 - x
                    index = (((hi >> bit) & 1) << 1) | ((lo >> bit) & 1)
                    pixels[(ty * 8 + y) * width + tx * 8 + x] = 255 - index * 85
            tile += 1
    return pixels
...
def main() -> None:
    ...
    report = {
        "batch": "002",
        "category": "player fishing overworld graphics",
